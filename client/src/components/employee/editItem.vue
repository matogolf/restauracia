    ...
    <v-flex xs8>
      ...
      <v-btn
       ...
       <v-dialog v-model="dialog" max-width="310">
        <v-card>
         <v-card-title class="headline">Položka upravená</v-card-title>
          <v-card-text>Chcete prejsť späť do menu alebo dodatočne upraviť položku?</v-card-text>
          <v-card-actions>
          <v-spacer></v-spacer>
...
export default {
  data () {
    return {
      ...
      newItem: {
        ...
        allergens: this.$route.params.item.allergens,
        grammage: this.$route.params.item.grammage,
        volume: this.$route.params.item.volume,
        alcohol: this.$route.params.item.alcohol,
        itemCategoryId: this.$route.params.item.itemCategoryId,
        menuId: this.$route.params.item.menuId
      },
      error: null,
      done: false,
      number: [
  ...
  methods: {
    ...
    editItem () {
      ...
      const response = MenuServices.itemEdit(this.newItem)
      console.log(response.data)
      this.dialog = true
